...
for i=1:nvar
    ...
    switch type
        ...
        case 6 % marker
            ...
            for ii=1:NMarkers
                nexFile.markers{markerCount,1}.values{ii,1}.name = deblank(char(fread(fid, 64, 'char')'));
                %prelocate memory for strings -- WW2011
                nexFile.markers{markerCount,1}.values{ii,1}.strings = cell(n,1);
                for p = 1:n
                    nexFile.markers{markerCount,1}.values{ii,1}.strings{p, 1} = deblank(char(fread(fid, MarkerLength, 'char')'));
                end
            end
            fseek(fid, filePosition, 'bof');
