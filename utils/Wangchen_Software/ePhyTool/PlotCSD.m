function hfig = PlotCSD(csd,varargin)
...
%              if ~isdmatrix(lfp) | size(lfp,1) ~= size(csd,1) | size(lfp,2) ~= size(csd,2)+2,
             if size(lfp,1) ~= size(csd,1) | size(lfp,2) ~= size(csd,2)+2,
 ...
 hfig = figure('name','CSD'); 
 
 pcolor(csd(:,1)*1000,((1:size(d,2))+1)/2+1,flipud(transpose(d))); shading interp;
 %pcolor(csd(:,1)*1000,1:size(d,2),(transpose(d))); shading interp; %used
 %for 'output2' results which arranged toppest channels at the bottom of
 %graph. correcte by transposing the matrix with pcolor. 
...
%          plot(lfp(:,1),y(:,i)+(n-i)*2-1,'k');
           plot(lfp(:,1)*1000,y(:,i)+(n-i)+1,'k');
     end
     %ylim([-2 (n-1)*2]);

 end
 
     %change the labels to match the channel indices.
     %yTick = -1 : 4 : 2*n-3;
     yTick = 2 : 4 : size(d,2)+1;
     %yTick = get(gca,'YTick');
     %yTick = round(yTick/2)*2-1;
     %yTickLabel = (yTick+1)/2 +1 ;
     set(gca,'YTick',yTick);
     %set(gca,'YTickLabel',num2str(yTickLabel'));

 xlabel('Time(msec)'); ylabel('CSD');