...
%    s.nexData
%    s.nevData
%note: s is a single array,i.e, length(s)=1. s.nevData.events is 1-sized only and timestamps are ISI filtered.
...
m = length(d.markers);
for i = 1 : m
    if strcmp(d.markers{i}.name,'StimMarker')
        marker = d.markers{i};
        break;
    end
end
...
%         LUT(:,i) = str2double((marker.values{i}.strings)); %column vector containing encoding index
          LUT(:,i) = cellfun(@str2double,(marker.values{i}.strings)); %column vector containing encoding index
    end
...
timestamps = d.events{1}.timestamps;
