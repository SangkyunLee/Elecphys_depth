function S= makeStimRows(Stim, n, flag,direction);
...
%add the flag for the extraction direction for flag = vector option.
if ~exist('direction','var'); direction = -1; end; %extraction before spike by default
...
elseif (length(flag) == 1)  % compute only for stimuli at times >= n ----------
    ...
    % Do for spinds < n
    if direction == -1 %extract vectors before spikes
    ...
    else %extract vectors after the spikes.
        %remove the spikes near the border, i.e, the vectors length shorter
        %than the kernel size, in the input (flag) -ww.
        for j = 1 : length(flag)
            S(j,:) = reshape(Stim(flag(j):flag(j)+n-1,:), 1, n2*n);
        end
    end
end
