...
%use dual display. if false, use primary display only and skip secondary display.
%dual-display didn't work with screen flip ! -WW2009 
dualDispMode = false ; 

%use bar as stimulus
allStimTypes = {'bar','dot'};
...
%reset display mode if only single display available
if length(allScreens) < 2
    dualDispMode = false;
end

%by default, primary display is set to 0. 
priScreen = min(allScreens);
    
if dualDispMode
    secScreen = max(allScreens);
end    
...
%open window 
pWin = Screen('OpenWindow',priScreen, bgColor);

if dualDispMode
    sWin = Screen('OpenWindow',secScreen, bgColor);
end
...
%check the settings on both screens.
if dualDispMode
    sRefRate = Screen('NominalFrameRate',sWin);
    sRect = Screen('rect',sWin);
    
    if ~( (pRefRate == sRefRate) && all(pRect == sRect) )
        error('Different Monitor Settings Detected. Adjust To The Same');
        Screen('CloseAll');
        return;
    end
end
...
%try
    ...
    %show the initial stim
%     Screen('DrawTexture',pWin,pIdxOutline,[],outlinePos);
...
%         %if no keypressed or no mouse click, recheck.
%         if ~isKeyDown && ~isMouseDown && ~isMouseMoved 
% 
%             %insert a waittime to prevent cpu hogging
%             WaitSecs(0.001);
%             continue;
%         end
...
%                      continue;
                 ...
                 otherwise
                     ...
                     %WaitSecs(0.01);
                     %if no mouse click or movement, go on to next from here.
%                      if ~isMouseDown && ~isMouseMoved
%                          %fprintf('not support key function for %s\n',KbName(kcodePress));
%                          continue;
%                      end
...
%        keep flashing if nothing goes on.         
%         %if neither key or mouse down or move, skip to beginning of loop.
%         if ~(isKeyDown || isMouseDown || isMouseMoved)
%             continue;
%         end
...
% 
%             Screen('DrawTexture',pWin,pIdxOutline,[],outlinePos);
% draw dots so that we could change color
             Screen('DrawDots',pWin,...
           outlineMatrix,1,outlineColor,outlineCenter,dotType);
        ...
        if isStimShown
            ...
            %show stimulus with drawdots function
%             Screen('DrawDots',pWin,stimMatrix,2,repmat(flashColorVal,3,1),stimCenter,dotType);
            %or show with texture
            Screen('DrawTexture',pWin,stimShowTex{stimShowTexIdx},stimSrcRect,stimPos,stimOrient);
