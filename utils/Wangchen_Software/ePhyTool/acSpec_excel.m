...
% %waveform info table --- 11 entries
% Channel
% Unit
% Timestamp
% PC1
...
if isempty(waveFileFullPath)
    lastSession.waveFile = strrep(lastSession.waveFile,'.mat','.xls');
...
chList = unique(d(:,Index.Channel));
for i = 1 : nChan
    subplot(nRow,nCol,i);
%     ch = d.(chNames{i});
    if i > length(chList); continue; end
    ch = d(find(d(:,Index.Channel)==chList(i)),:);
    nSpikes = size(ch,1);
    if nSpikes < 1
        continue;
    end
    ...
    if showAllUnit
        sortUnit = find(ch(:,Index.Unit)>=0 & ch(:,Index.Unit)~=255);
    else
        sortUnit = find(ch(:,Index.Unit)==1 & ch(:,Index.Unit)~=255);
    end
