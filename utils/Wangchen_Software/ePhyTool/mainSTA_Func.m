...
popup = false; %use gui to locate folder
...
if loadClustered
    ...
    [rowI colI] = ind2sub([6 4], 1:24);
    cmap = [rowI-1 ; colI ; 1:24]';
...
%     tune_var = encode_vars{tune_vid};
    ...
    %checkChannel = [1:24];  %select channels for the plot.
    ...
    viewOption.saveFolder = s.nevFolder;
...
%     %smooth data
%     neurons_fit = neurons;
% %     opts_fit.errorbars = 'bootstrap';
%     opts_fit.errorbars = 'none';
%     %do a gassuan fit 
%     for ii = 1 : length(neurons)
%         for jj = 1 : length(neurons{ii}.clusters)
%             for kk = 1 : length(neurons{ii}.clusters{jj}.class{1}.member)
%                 [nx,ny,nt] = size((neurons{ii}.clusters{jj}.class{1}.member{kk}.sta));
%                 [x,y] = meshgrid(1 : nx, 1: ny);
%                 z = neurons{ii}.clusters{jj}.class{1}.member{kk}.sta;
%                 for ll = 1 : nt
%                     zt = z(:,:,ll);
%                     zmax = max(max(zt));
%                     zmin = min(min(zt));
%                     zstd = std(reshape(zt,1,[]));
%                     zavg = mean(reshape(zt,1,[]));
%                     g = autoGaborSurf(x,y,zt,opts_fit);
%                     fitType = 'Gabor';
% %                     if zmax-zavg > 2*zstd && zavg-zmin> 2*zstd 
% %                         g = autoGaborSurf(x,y,zt);
% %                         fitType = 'Gabor';
% %                     else
% %                         g = autoGaussianSurf(x,y,zt);
% %                         fitType = 'Gaussian';
% %                     end
%                     neurons_fit{ii}.clusters{jj}.class{1}.member{kk}.sta(:,:,ll) = g.G;
%                     neurons_fit{ii}.clusters{jj}.class{1}.member{kk}.fit{ll} = g;
%                     neurons_fit{ii}.clusters{jj}.class{1}.member{kk}.fitType{ll} = fitType;
%                 end
%             end
%         end
%     end
    
    viewOption.plot = 'STA_FIT';
    
    %h_STA_fit = mspecViewer(xSTA,neurons_fit,viewOption);
    
    matDataFile = fullfile(s.nevFolder,sprintf('RF.mat'));
    
    %reduece the data size.
    %clear neurons neurons_fit;
    clear s ss s_SETS;
    
    save(matDataFile); %save workspace to mat file.
