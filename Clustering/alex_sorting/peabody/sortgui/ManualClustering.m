...
function ManualClustering_OpeningFcn(hObject, ~, handles, model, filename, autoact)
...
if nargin > 4
    set(gcf,'Name',[mfilename, ' || ', filename]);
end

%user input mode. (default : no auto action)
if nargin < 6
    autoact = 'none';
end

switch lower(autoact)
    case 'skip'
        autoTag = 'opSkip';
    case 'accept'
        autoTag = 'opAccept';
...
if ~isempty(handles.autoTag)
    hgfeval(get(findobj('Tag',handles.autoTag),'callback'),findobj('Tag',handles.autoTag),[]);
else
...
if ~ishandle(hObject)
    ...
    if isempty(handles.autoTag) %keep figure alive if autoact is enabled
...
[fp fn snr frac isi] = getStats(handles.modelData);
su = hasTag(handles.modelData, 'SingleUnit');
dt = hasTag(handles.modelData, 'DoubleTriggered');
set(handles.stats, 'Data', num2cell([clusIds; groups; 100 * fp; 100 * fn; snr; 100 * frac; su; dt; 100 * isi]'));
