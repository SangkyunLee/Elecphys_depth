...
%             params.binSize = 0.5; %in msec
            params.nBins = 80;
            params.binSize = 30 / params.nBins ; %change default ccg to higher resolution.
            ...
            for i = 1 : N
                ...
                for j = 1 : N
                    ...
                    if i == j
                        ...
                        yl = ylim;
                        text(0,diff(yl)/2,num2str(i),'Color',[1 1 1])
                    else
                        hold on
                    ...
                    plot([-2 -2],ylim,'Color',[0.7 0.7 0.7],'LineWidth',0.5);
                    plot([2 2],ylim,'Color',[0.7 0.7 0.7],'LineWidth',0.5);
        ...
        function [fp fn snr frac isi] = getStats(self,varargin)
            ...
            % JC 2009-09-27
            % 
            % add isi statictis (fraction within 2ms) 
            % WW 2013-11-05
            ...
            for i = 1:length(params.clusIds)
                index = getSpikesByClusIds(self, params.clusIds(i));
                ids{i} = index;
                if isempty(ids{i}), isi(i) = NaN; continue; end
                spikes = self.SpikeTimes.data(index); %times in ms.
                dt = diff(spikes); 
                if isempty(dt), isi(i) = NaN; continue; end
                isi(i) = numel(find(dt < 2 & dt > 0.3))/numel(dt) ; %refractory period & non-double-triggered 
        ...
        function plotLDAs(self, varargin)
            ...
            for i = 1:length(params.clusIds)
                for j = i+1:length(params.clusIds)
                    ...
                    hold on;
                    plot(bins,n1,'Color',color(i,:)*0.8);
                    plot(bins,n2,'Color',color(j,:)*0.8);
